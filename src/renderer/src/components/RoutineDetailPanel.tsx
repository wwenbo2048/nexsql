...
import { FunctionSquare, Loader2, Copy, Check, Info, User, Shield, Clock, ArrowLeftRight, Code } from 'lucide-react'
...
export default function RoutineDetailPanel() {
  ...
  const { selectedConnectionId, selectedDatabase, selectedTable, routines } = useBrowserStore()
  ...
  const routineInfo: RoutineInfo | undefined = routines.find((r) => r.name === selectedTable)

  const loadDDL = useCallback(async () => {
    if (!config || !selectedDatabase || !selectedTable || !routineInfo) return
    setDdlLoading(true)
    try {
      const sql = routineInfo.type === 'FUNCTION'
        ? `SHOW CREATE FUNCTION \`${selectedTable}\``
        : `SHOW CREATE PROCEDURE \`${selectedTable}\``
  ...
  }, [config, selectedDatabase, selectedTable, routineInfo])
  ...
  if (!selectedTable) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-text-muted bg-bg-primary">
        <FunctionSquare size={48} className="opacity-20 mb-3" />
        <p className="text-sm">选择一个函数或存储过程</p>
      </div>
    )
  }
        ...
        {activeTab === 'info' && (
          <div className="h-full overflow-auto p-4">
            <div className="flex items-center gap-2 mb-4 pb-3 border-b border-border-light">
              <FunctionSquare size={16} className={routineInfo?.type === 'FUNCTION' ? 'text-orange-400' : 'text-yellow-400'} />
              <span className="text-sm font-medium text-text-primary">{selectedTable}</span>
              {routineInfo?.type && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${routineInfo.type === 'FUNCTION' ? 'text-orange-400 bg-orange-950/40' : 'text-yellow-400 bg-yellow-950/40'}`}>
                  {routineInfo.type}
                </span>
              )}
            </div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-0">
              <InfoRow icon={<Info size={13} />} label="名称" value={routineInfo?.name ?? '-'} />
              <InfoRow icon={<ArrowLeftRight size={13} />} label="类型" value={routineInfo?.type ?? '-'} />
              {routineInfo?.type === 'FUNCTION' && (
                <InfoRow icon={<ArrowLeftRight size={13} />} label="返回类型" value={routineInfo?.returnType ?? '-'} />
              )}
              <InfoRow icon={<User size={13} />} label="定义者" value={routineInfo?.definer ?? '-'} />
              <InfoRow icon={<Shield size={13} />} label="安全" value={routineInfo?.security ?? '-'} />
              <InfoRow icon={<Clock size={13} />} label="创建时间" value={routineInfo?.created ?? '-'} />
              <InfoRow icon={<Clock size={13} />} label="修改时间" value={routineInfo?.modified ?? '-'} />
            </div>
            {routineInfo?.comment && (
              <div className="mt-4 pt-3 border-t border-border-light">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-primary mt-0.5">{routineInfo.comment}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="h-full flex flex-col">
            <div className="flex items-center justify-between px-3 py-1 border-b border-border-light bg-bg-secondary flex-shrink-0">
              <span className="text-xs text-text-muted">
                {routineInfo?.type === 'FUNCTION' ? 'SHOW CREATE FUNCTION' : 'SHOW CREATE PROCEDURE'}
              </span>
